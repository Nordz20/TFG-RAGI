...
def search_hibrido(es, query, top_k=TOP_K):
    pool_size = max(30, top_k * 2)
    lexical_results = search_ragi_basico(es, query, top_k=pool_size)
    semantic_results = search_ragi_avanzado(es, query, top_k=pool_size)
    
    rrf_scores = {}
    k = 60
    
    for rank, img_path in enumerate(lexical_results, 1):
        rrf_scores[img_path] = rrf_scores.get(img_path, 0.0) + 1.0 / (k + rank)
        
    for rank, img_path in enumerate(semantic_results, 1):
        rrf_scores[img_path] = rrf_scores.get(img_path, 0.0) + 1.0 / (k + rank)
        
    sorted_results = sorted(rrf_scores.items(), key=lambda x: x[1], reverse=True)
    return [img_path for img_path, score in sorted_results][:top_k]
...
def evaluate():
    ...
    for idx, (query, expected_img) in enumerate(queries, 1):
        ...
        retrieved_hibrido = search_hibrido(es, query, top_k=TOP_K)
        ...
        calculate_metrics(retrieved_hibrido, "hibrido")

    print("\n" + "="*50)
    print("📊 RESULTADOS FINALES DE LA EVALUACIÓN CUANTITATIVA")
    print("="*50)
    
    for model in ["baseline", "basico", "avanzado", "hibrido"]:
